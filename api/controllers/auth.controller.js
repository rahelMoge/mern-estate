...
export const signin = async (req, res, next) => {
  ...
  try {
    ...
    res
      .cookie("access_token", token, { httpOnly: true, path: '/', maxAge: 30 * 24 * 60 * 60 * 1000 }) // Store JWT in secure cookie
      .status(200)
...
export const google = async (req, res, next) => {
  ...
  try {
    ...
    if (user) {
      ...
      res
        .cookie('access_token', token, { httpOnly: true, path: '/', maxAge: 30 * 24 * 60 * 60 * 1000 }) // Save token in cookie
        .status(200)
      ...
      res
        .cookie('access_token', token, { httpOnly: true, path: '/', maxAge: 30 * 24 * 60 * 60 * 1000 }) // Store token in cookie
        .status(200)
